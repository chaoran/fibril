...
static void remap(struct _fibril_t * frptr)
{
  ...
  *(void **) addr = addr + size;
  ...
  SAFE_NNCALL(mprotect(protaddr, protsize, PROT_READ | PROT_WRITE));
#endif
}
...
#if STATS_STACK_PAGES != 0
  void * old_top = *(void **) addr;
  void * new_top = addr + PARAM_STACK_SIZE;
  long diff = ((long) (new_top - old_top)) / PARAM_PAGE_SIZE;
  STATS_COUNTER_DEC(STATS_STACK_PAGES, diff);
#endif
...
void stack_init(int id)
{
  ...
  enqueue(alloc(NULL, PARAM_STACK_SIZE));
...
#if STATS_STACK_PAGES != 0
  void * addr = PARAM_STACK_ADDR + PARAM_PAGE_SIZE;
  void * top  = *(void **) PARAM_STACK_ADDR;
  size_t size = top - addr;
  int flags = MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS | MAP_NORESERVE;
  SAFE_NNCALL(mmap(addr, size, PROT_NONE, flags, -1, 0));
#endif
}
